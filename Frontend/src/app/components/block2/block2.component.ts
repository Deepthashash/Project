...
export class Block2Component implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.fileService.getAllFilesBlock2().subscribe(
      (res) => {
        this.files = res;
    ...
    this.commentService.getAllCommentsBlock2().subscribe(
      (res) => {
        this.comments = res;
      }
    )
  ...
  submit(formData: Comments){
    ...
    this.commentService.postComment({...formData, userId: this.user._id, blockName: "block2"}).then(
      (result)=>{
        console.log(result);
        location.reload();
      },
